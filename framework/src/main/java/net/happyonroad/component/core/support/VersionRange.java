...
import net.happyonroad.component.core.exception.OverConstrainedVersionException;
...
public class VersionRange
{
    ...
    public ComponentVersion getRecommendedVersion()
    {
        ...
        throws InvalidVersionSpecificationException
    {
        ...
        else
        {
            ...
            ComponentVersion lowerVersion = null;
            if ( lowerBound.length() > 0 )
            {
                lowerVersion = new ComponentVersion( lowerBound );
            }
            ComponentVersion upperVersion = null;
            if ( upperBound.length() > 0 )
            {
                upperVersion = new ComponentVersion( upperBound );
            }
    ...
    private List<Restriction> intersection( List r1, List r2 )
    {
        ...
        while ( !done )
        {
                ...
                res1.getLowerBound().compareTo( res2.getUpperBound() ) <= 0 )
            {
                ...
                {
                    ComponentVersion lower;
                    ComponentVersion upper;
    ...
    public ComponentVersion getSelectedVersion( Component component)
        throws OverConstrainedVersionException
    {
        ...
        else
        {
            if ( restrictions.size() == 0 )
            {
                throw new OverConstrainedVersionException( "The Component has no valid ranges", component);
            }
    ...
    public boolean isSelectedVersionKnown( Component component)
        throws OverConstrainedVersionException
    {
        boolean value = false;
        if ( recommendedVersion != null )
    ...
    public ComponentVersion matchVersion( List versions )
    {
        // TODO: could be more efficient by sorting the list and then moving along the restrictions in order?

        ComponentVersion matched = null;
        for (Object version1 : versions) {
            ComponentVersion version = (ComponentVersion) version1;
    ...
    public boolean containsVersion( ComponentVersion version )
    {
        for (Object restriction1 : restrictions) {
            Restriction restriction = (Restriction) restriction1;
